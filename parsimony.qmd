...
The parsimony, by helping avoiding *hasardous predictions* when the features go away from the domain spanned by the training data, improves the quality of the detection as it is suggested in the following sketchy representation:
...
![Because the model (in Green) is not parsimonuous, the probability it overfits the data is greater making the diagnosis of the red circle more difficult to assess with confidence.](images/parsimony2){width=80%}
:::
...
While this simple example highlights the relevance of sparsity in **detecting true anomalies**, the following discussion focuses on its role in **avoinding false alarms** upon changes in the context of use. 


More precisely, the need for sparsity in the industrial case stems from two major specificities, namely: The curse of contexts and the aversion to false alarms. These issues are explained hereafter.
...
The multiplicity of contexts of use and their highly probable absence in the training data enhances the risk of raising **false alarm** because of the resurgence of an unseen context rather than because of a real change in the system's integrity or health parameters. 
...
: Compared to a bad classification in images or an erroneous book/movie recommendation:
...
This means that with a **false alarm rate of 1%**, whcih is considered to be quite small in the everyday applications mentioned above, one should expect more than **28 false alarms per day** which is obviously inacceptable and might lead to the alarm system being totally deactivated by the operators. Even with a false alarm rate of 0.1\% this might lead to almost 3 false alarm per day which is still unacceptable. 

## Sparsity and physics 

knowledge based physical laws are known to be a major source of sparsity. As an example, consider the most famous physical law one can imagine, namely, the Newton's law in its translational form: 
...
With this new set of measurements, it is @eq-newton2 that represents the normality of the system in a way that is still context-independent.

From the above simple and intuitive example, it comes out that: 

:::{.callout-note title="physical laws are sparse"}
Almost any context of use might be sufficient to capture the values of the parameters $(m, k_1, k_2, \nu)$ involved in @eq-newton2 leading to a relationship that holds over **all other unseen conttexts**. Therefore no alarm would be raised when these unseen contexts arise because the residual remains small. 
:::
...
The [following section](parsimony_oscillator.qmd) shows an example of physical system highlighting the difference between a **blind normality characterization** and the one based on parsimonious **physically-inspired normality characterization**.
