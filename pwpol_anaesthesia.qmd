...
subtitle: Characterizing normality in anaesthesia-related sensors
---
...
:::{.column width=65%}
In this section, it is shown how the identification (via the `pwpol` module of the `MizoPol` package) of multivariate piece-wise polynomial invariants involving a set of sensors might be used to characterize the normality and hence help **detecting abnormal events during intervention in the operation room**. 
...
Obviously, these goals can be progressively achieved by iterations between the algorithm and experts annotating/correcting the early labelling by the automatic tool. This is sketched in the following scheme: 
...
Nevertheless, it is worth emphasizing that the methodology is perfetcly generic and might be successfully applied to any extended set of sensors[^sampling] (and not necessarily those linked to anaesthesia).
...
The previously defined normality characterization might be used to fire alarms in the operation rooms. The following figures show the resulting events detection statistics for different levels of acceptable false alarm rates and different decision window's size. 
...
![](images/health_w120.png)

### window = 1min

![](images/health_w60.png)

### window = 30 sec

![](images/health_w30.png)

### window = 10 sec

![](images/health_w10.png)
