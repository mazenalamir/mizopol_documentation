...
This obviously splits the space (via the condition $\vert y-\texttt{y\_{ref}}\vert\le \epsilon$) into two regions over each of which the relationship between the features $(y,\texttt{y\_ref})$ and the targeted label $u$ is polynomial (linear in the specific example) but there is no a single polynomial relationship that matches @eq-conditionalgain on the whole space of features. 
...
In other words, in the absence of $y$, there is no way to determine which one of the values $P_i(x)$ is the closest to the true value $y$. Nevetheless, whey $(x,y)$ is available, a residual (distance to normality) can be computed by @eq-implicit-pwpol.
:::
...
:::
 ...
 The reasons for which parsimony is a key element in the context of processing industrial and technological time-series are discussed in the [parsimony-dedicated section](parsimony.qmd). 
