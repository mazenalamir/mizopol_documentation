...
$$
\dfrac{\vert y-\hat y\vert }{\texttt{median}(\vert y\vert )}\le \texttt{th}
$$
Notice however that the value of the threshold $\texttt{th}$ is **first set** to the corresponding argument of the `dic_args` used in the call of the `fit` method but **it might be increased** after a given number of failures, this explains the following log content where the final percentile needed a higher value of the threshold.
:::
