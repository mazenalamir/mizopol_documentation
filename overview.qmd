...
**Dynamic**
$$
\Delta^{(m_i)} s_i(k)=F_i\left(\begin{array}
\{s_i(k-\sigma\times d_i)\}_{\sigma\in \{1,\dots,m_i\}}\cr 
\{s_j(k-\sigma\times d_i)\}_{(j\neq i,\sigma\in \{0,\dots,m_i\})}
\end{array}
\right)
$$
$\rightarrow$ The increment $\Delta^{(m_i)} s_i(k)$ represents the discrete version, at instant $k$, of the $m_i$ derivatives of $s_i$.
:::
